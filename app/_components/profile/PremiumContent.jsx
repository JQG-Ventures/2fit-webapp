...
const Premium = ({ onClose }) => {
    ...
    if (isDesktop) {
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-500 opacity-100">
                <div
                    className="relative bg-white max-w-4xl rounded-3xl shadow-lg p-10 bg-cover bg-center bg-no-repeat"
                    style={{
                ...
                >
                    ...
                    <div className="relative z-10 bg-white/80 rounded-3xl p-4">
                        <div className="relative w-full max-w-5xl flex flex-col items-start justify-center space-y-8 overflow-hidden">
                            <div className="h-[20%] md:pb-2 md:mb-2">
                                <div className="flex flex-col items-start space-y-5 pb-10 md:pb-6 md:pt-10">
                                    ...
                                    <p className="text-3xl text-gray-700">{t('BePremium.description')}</p>
                                </div>
                            </div>
                            ...
                            <div className="w-full space-y-8 mt-6 md:mt-2 h-[40%]">
                                {['1', '2', '3'].map((plan) => (
                                    <div
                                        key={plan}
                                        className={`border-2 ${selectedPlan === plan ? 'border-green-700 bg-green-300/20' : 'border-gray-200 bg-white'} rounded-2xl p-8 flex items-center justify-between cursor-pointer transition-transform duration-300 shadow-lg`}
                                    ...
                                    >
                                        <input
                                            ...
                                            readOnly
                                        />
                                        <label htmlFor={`plan${plan}`} className="flex items-center w-full cursor-pointer justify-between">
                                            <span className="w-8 h-8 inline-flex items-center justify-center rounded-full border-2 border-green-700 mr-4">
                                                <span
                                                    className={`w-4 h-4 rounded-full transition-opacity duration-200 ${selectedPlan === plan ? 'bg-green-700 opacity-100' : 'opacity-0'}`}
                                                ></span>
                                            </span>
                                            <div className="flex flex-col w-full">
                                                <div className="flex items-center justify-between">
                                                    <h2 className="font-semibold text-4xl">
                                                        {plan === '1' ? t('BePremium.plans.1.name') : plan === '2' ? t('BePremium.plans.2.name') : t('BePremium.plans.3.name')}
                                                    </h2>
                                                    <h2 className="font-semibold text-4xl text-gray-800">
                                                        {plan === '1' ? t('BePremium.plans.1.price') : plan === '2' ? t('BePremium.plans.2.price') : t('BePremium.plans.3.price')}
                                                    </h2>
                                                </div>
                                                <p className="text-2xl text-gray-600">{t('BePremium.note')}</p>
                                            </div>
                                        </label>
                                    </div>
                                ))}
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-50">
            <div className="relative w-full h-full bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('/images/onboarding-3.jpg')" }}>
                <button
                    className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 focus:outline-none"
                    onClick={onClose}
                >
                        ...
                        <div className="pb-16">
                            ...
                            <div className="w-full space-y-5 mt-6 md:mt-2">
                                {['1', '2', '3'].map((plan) => (
                                    <div
                                        key={plan}
                                        className={`border-2 ${selectedPlan === plan ? 'border-green-700 bg-green-300/20' : 'border-gray-200 bg-white'} rounded-2xl p-8 flex items-center cursor-pointer transition-transform duration-300 shadow-lg`}
                                        onClick={() => setSelectedPlan(plan)}
                                    >
                                        <input
                                            type="radio"
                                        ...
                                        <label htmlFor={`plan${plan}`} className="flex items-center w-full cursor-pointer justify-between">
                                            <span className="w-8 h-8 inline-flex items-center justify-center rounded-full border-2 border-green-700 mr-4">
                                                <span className={`w-4 h-4 rounded-full transition-opacity duration-200 ${selectedPlan === plan ? 'bg-green-700 opacity-100' : 'opacity-0'}`}></span>
                                            </span>
                                            <div className="flex flex-col w-full">
                                                <div className="flex items-center justify-between">
                                                ...
                                                <p className="text-xl text-gray-600">{t('BePremium.note')}</p>
                                            </div>
                                        </label>
                                    </div>
                                ))}
