...
const WorkoutCard = ({ title, workoutCount, description, image }) => (
    <div
        className="relative card bg-white p-6 rounded-xl shadow-md mb-6 lg:mb-0 lg:w-full overflow-hidden transition-transform duration-300 ease-in-out transform hover:scale-105 hover:shadow-xl active:scale-95"
    >
        ...
        ></div>
        <div className="absolute inset-0 bg-gradient-to-r from-black opacity-100 to-transparent transition-opacity duration-300 ease-in-out"></div>
...
const WorkoutLibrarySection = ({ workouts }) => {
            ...
            <div className="relative z-10 p-6 lg:p-12">
                ...
                <div className="block lg:hidden mb-12">
                    <div className="flex flex-col gap-6">
                        {displayedWorkoutsCol.map((workout, index) => (
                            <WorkoutCard 
                                key={index} 
                                title={workout.title} 
                ...
                <div className="hidden lg:grid lg:grid-cols-3 lg:gap-6 mb-12">
                    {displayedWorkoutsGrid.map((workout, index) => (
                        <WorkoutCard 
                            key={index} 
                            title={workout.title} 
                            workoutCount={workout.workoutCount} 
