...
def test_get_by_email_found(db, sample_user) -> None:
    repo = UserRepository()
...
def test_get_by_email_not_found(db) -> None:
    repo = UserRepository()
    assert repo.get_by_email("nonexistent@example.com") is None


def test_get_by_number_found(db, sample_user) -> None:
    repo = UserRepository()
...
def test_get_by_number_not_found(db) -> None:
    repo = UserRepository()
    assert repo.get_by_number("+19999999999") is None


def test_get_by_id_found(db, sample_user) -> None:
    repo = UserRepository()
...
def test_get_by_id_not_found(db) -> None:
    repo = UserRepository()
    assert repo.get_by_id(uuid.uuid4()) is None


def test_update_profile_updates_fields(db, sample_user) -> None:
    repo = UserRepository()
...
def test_update_profile_not_found(db) -> None:
    repo = UserRepository()
    assert repo.update_profile(uuid.uuid4(), {"name": "X"}) is None


def test_create_with_related_creates_preference_settings_automation(db) -> None:
    repo = UserRepository()
    ...
    assert (
        db.session.scalar(select(UserSettings).where(UserSettings.user_id == user.id)) is not None
    )
    assert (
        db.session.scalar(select(UserAutomationData).where(UserAutomationData.user_id == user.id))
        is not None
    )


def test_get_all_empty_filter(db) -> None:
    repo = UserRepository()
