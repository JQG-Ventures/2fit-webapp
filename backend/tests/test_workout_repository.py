...
def test_get_active_plans_excludes_personalized(app, db) -> None:
...
def test_get_with_schedule_loads_days_and_exercises(app, db, plan_with_one_exercise) -> None:
    with app.app_context():
        repo = WorkoutPlanRepository()
...
def test_get_one_day_plans_only_single_day_plans(app, db) -> None:
...
def test_get_by_difficulty(app, db) -> None:
...
def test_soft_delete(app, db, sample_workout_plan) -> None:
...
def test_get_library_with_exercise_count(app, db, plan_with_one_exercise) -> None:
    with app.app_context():
        repo = WorkoutPlanRepository()
...
def test_create_full_plan_persists_schedule(app, db) -> None:
    ex = ExerciseFactory.create()
    with app.app_context():
        repo = WorkoutPlanRepository()
...
def test_replace_schedule(app, db, plan_with_one_exercise) -> None:
