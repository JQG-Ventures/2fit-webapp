...
executed_exercise_model = api.model('ExecutedExercise', {
    ...
    'is_completed': fields.Boolean(default=True, description='Whether the exercise was completed'),
})

completed_workout_model = api.model('CompletedWorkout', {
    ...
    'exercises': fields.List(fields.Nested(executed_exercise_model), required=True, description='List of completed exercises'),
    'day_of_week': fields.Integer(),
    'sequence_day': fields.Integer(),
    'was_skipped': fields.Boolean(required=True)
})

progress_response_model = api.model('ProgressResponse', {
...
response_model = api.model('ErrorModel', {
    'status': fields.String(description='Status of the request'),
    'message': fields.String(description='Error message')
})

progress_success_response_model = api.model('ErrorModel', {
...
progress_update_model = api.model('ProgressUpdate', {
    'exercises': fields.List(fields.Nested(api.model('ExerciseProgress', {
        'exercise_id': fields.String(required=True, description='ID of the exercise'),
        'sets_completed': fields.Integer(required=True, description='Number of sets completed'),
        'reps_completed': fields.List(fields.Integer, required=True, description='List of reps completed for each set'),
        'duration_seconds': fields.Integer(required=True, description='Duration spent on the exercise in seconds'),
        'calories_burned': fields.Float(required=True, description='Calories burned during the exercise'),
        'is_completed': fields.Boolean(required=True, description='Whether the exercise is completed')
    })), required=True, description='List of exercises with progress updates'),
...
profile_image_response_model = api.model('ProfileImageResponse', {
    'status': fields.String(description='Status of the request'),
    ...
    def post(self):
        ...
        try:
            ...
            try:
                ...
                return {'status': 'error', 'message': 'Invalid input', 'errors': err.messages}, 400
    ...
    def get(self):
        ...
        try:
            ...
            if not workout_plan_id:
                return {'status': 'error', 'message': 'Missing required parameter: workout_plan_id'}, 400
    ...
    def post(self):
        ...
        try:
            ...
            try:
                ...
                return {'status': 'error',
                        'message': 'Invalid date format. Expected ISO format: YYYY-MM-DDTHH:MM:SS.mmmmmm'}, 400
...
motivational_messages_response_model = api.model('MotivationalMessagesResponse', {
    'status': fields.String(description='Status of the request'),
...
class MotivationalMessagesResource(Resource):
    @api.response(200, 'Motivational messages retrieved successfully', motivational_messages_response_model)
...
send_code_model = api.model('SendCode', {
    'number': fields.String(required=True, description='User phone number', example='+1234567890')
})
...
class SendCodeResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            client.verify.v2.services(s.TWILIO_SERVICE_SID)\
            ...
            return {"status": "error", "message": f"Twilio could not send the code: {te}"}, te.status
        except Exception as e:
            logger.exception(f"There was a problem sending the verification code: {e}")
            return {"status": "error", "message": f"There was a problem sending the verification code: {e}"}, 500
...
conversation_response_model = api.model('ConversationResponse', {
    'status': fields.String(description='Status of the request'),
...
class UserConversationResource(Resource):
    ...
    @jwt_required()
    @api.response(200, 'User conversation retrieved successfully', conversation_response_model)
...
class UserProfileImageResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            if updated:
                return {"status": "success", "message": "Profile image updated successfully", "url": blob_url}, 200
            else:
