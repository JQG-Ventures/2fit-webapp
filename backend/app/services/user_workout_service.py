...
class UserWorkoutService:
    ...
    def _fetch_active_plan(user: dict, workout_plan_id: str):
        ...
        active_plan = next(
            (plan for plan in user.get('workout_history', {}).get('active_plans', [])
    ...
    @staticmethod
    def _update_progress_details(active_plan: dict, completed_workout: dict, workout_day: dict, week_number: int):
        ...
        day_identifier = completed_workout.get('day_of_week') or completed_workout.get('sequence_day')
        day_progress = next(
            (dp for dp in progress_details
             if (dp.get('day_of_week') == day_identifier or dp.get('sequence_day') == day_identifier)),
            None
        )
        ...
        for ex_id, ex_data in new_exercises.items():
            if ex_id in existing_exercises:
                ...
                existing_ex['sets_completed'] = max(existing_ex.get('sets_completed', 0),
                                                    ex_data.get('sets_completed', 0))
                existing_ex['reps_completed'] = existing_ex.get('reps_completed', []) + ex_data.get('reps_completed',
                                                                                                    [])
                existing_ex['duration_seconds'] += ex_data.get('duration_seconds', 0)
                existing_ex['calories_burned'] += ex_data.get('calories_burned', 0)
                existing_ex['is_completed'] = existing_ex.get('is_completed') or ex_data.get('is_completed')
            else:
                day_progress['exercises'].append(ex_data)

        scheduled_exercises = workout_day.get('exercises', [])
        scheduled_exercise_ids = {ex.get('exercise_id') or ex.get('exerciseId') for ex in scheduled_exercises}
        completed_exercise_ids = {ex['exercise_id'] for ex in day_progress['exercises'] if ex.get('is_completed')}
    ...
    def save_completed_workout(user_id: str, completed_workout: dict):
        ...
        try:
            ...
            logging.error(f"Completed workout {completed_workout}, could not be saved for user {user_id}")
    ...
    def _calculate_personalized_progress(active_plan, workout_plan):
        """Calculate progress and exercises left for personalized plans, considering partial progress."""
        progress_details = active_plan.get('progress_details', [])
        today = datetime.now()
        ...
        today_scheduled_day = next(
            (day for day in workout_plan.get('workout_schedule', [])
        ...
        today_progress = next(
                    (dp for dp in progress_details
             if dp.get('week_number') == current_week_number and dp.get('day_of_week') == today_weekday),
            None
        )
        ...
        if in_progress_workout and in_progress_workout.get('day_of_week') == today_weekday:
            workout_date = datetime.strptime(in_progress_workout.get('date'), "%Y-%m-%dT%H:%M:%S.%f").date()

            if workout_date == today.date():
                in_progress_exercises = in_progress_workout.get('exercises', [])
                for ex in in_progress_exercises:
                    existing_ex = next(
                        (e for e in today_progress.get('exercises', []) if e['exercise_id'] == ex['exercise_id']),
                        None
                    )
                    if existing_ex:
                        existing_ex['sets_completed'] = max(existing_ex.get('sets_completed', 0),
                                                            ex.get('sets_completed', 0))
                        existing_ex['reps_completed'] = existing_ex.get('reps_completed', []) + ex.get('reps_completed', [])
                        existing_ex['duration_seconds'] += ex.get('duration_seconds', 0)
                        existing_ex['calories_burned'] += ex.get('calories_burned', 0)
                        existing_ex['is_completed'] = existing_ex.get('is_completed') or ex.get('is_completed')
                    else:
    ...
    def _calculate_challenge_progress(active_plan, workout_plan):
        ...
        today_progress = next(
            (dp for dp in progress_details
             if dp.get('sequence_day') == current_sequence_day),
            {}
        )

        scheduled_exercises = today_scheduled_day.get('exercises', [])
        total_exercises = len(scheduled_exercises)
        completed_exercises_ids = {ex['exercise_id'] for ex in today_progress.get('exercises', []) if
                                   ex.get('is_completed')}

        exercises_left_for_today = [
            ex for ex in scheduled_exercises
            if ex['exercise_id'] not in completed_exercises_ids
        ]
    ...
    def get_user_progress(user_id: str, workout_plan_id: str):
        ...
        try:
            ...
            if plan_type == 'personalized':
                progress_percentage, exercises_left = UserWorkoutService._calculate_personalized_progress(
                    active_plan, workout_plan
                )
            elif plan_type == 'challenge':
                progress_percentage, exercises_left = UserWorkoutService._calculate_challenge_progress(
                    active_plan, workout_plan
                )
            elif plan_type == 'one_day':
                progress_percentage, exercises_left = UserWorkoutService._calculate_one_day_progress(
                    active_plan, workout_plan
                )
            else:
    ...
    def _calculate_one_day_progress(active_plan, workout_plan):
        ...
        progress_percentage = (completed_exercises_count / total_exercises) * 100 if total_exercises else 0.0
    ...
    def get_weekly_workout_progress(user_id: str):
        try:
            ...
            for day_name in days_of_week:
                day_workout = next((day for day in workout_plan.get('workout_schedule', [])
                                    if day.get('day_of_week') == day_name), None)
                if not day_workout:
                    continue

                total_scheduled_days += 1
                day_progress = next((dp for dp in progress_details
                                    if dp.get('day_of_week') == day_name and dp.get('week_number') == current_week_number),
                                    {})

                is_completed = day_progress.get('is_completed', False)

                # Collect exercises completed from progress_details
                completed_exercises_ids = {ex['exercise_id'] for ex in day_progress.get('exercises', [])
                ...
                if in_progress_workout and in_progress_workout.get('day_of_week') == day_name:
                    ...
                    if in_progress_date_str:
                        in_progress_date = datetime.strptime(in_progress_date_str, "%Y-%m-%dT%H:%M:%S.%f")
    ...
    def get_challenge_progress(user_id: str, challenge_id: str):
        try:
            ...
            active_plan = next(
                (plan for plan in user.get('workout_history', {}).get('active_plans', [])
                 if plan['workout_plan_id'] == challenge_id and not plan.get('is_completed')), None
            )
            if not active_plan:
            ...
            for day in workout_plan.get('workout_schedule', []):
                ...
                day_progress = next((dp for dp in progress_details if dp.get('sequence_day') == sequence_day), {})
                is_completed = day_progress.get('is_completed', False)
    ...
    def save_workout_progress(user_id: str, workout_plan_id: str, progress_data: dict):
        ...
        try:
            ...
            workout_day = next(
                (day for day in workout_plan['workout_schedule']
                if day.get('day_of_week') == day_identifier or
                day.get('sequence_day') == day_identifier),
                None
            )
            if not workout_day:
            ...
            if in_progress_workout:
                in_progress_day_identifier = in_progress_workout.get('day_of_week') or in_progress_workout.get(
                    'sequence_day')
                if (in_progress_day_identifier == day_identifier and 
                    in_progress_workout.get('date') == progress_data_copy.get('date')):
                    # Same day, merge exercises
                    existing_exercises = {ex['exercise_id']: ex for ex in in_progress_workout.get('exercises', [])}
                    new_exercises = {ex['exercise_id']: ex for ex in progress_data_copy.get('exercises', [])}
                    ...
                    previous_day_identifier = in_progress_workout.get('day_of_week') or in_progress_workout.get(
                        'sequence_day')

                    previous_workout_day = next(
                        (day for day in workout_plan['workout_schedule']
                        if day.get('day_of_week') == previous_day_identifier or
                        day.get('sequence_day') == previous_day_identifier),
                        None
                    )
                    ...
                    week_number = UserWorkoutService.calculate_week_number(active_plan['start_date'], in_progress_date)

                    UserWorkoutService._update_progress_details(
                        active_plan, in_progress_workout, previous_workout_day, week_number
                    )

                    active_plan['in_progress_workout'] = None
                    in_progress_workout = {
                        'workout_id': workout_plan_id,
                        'workout_name': progress_data_copy.get('workout_name', active_plan.get('workout_name')),
            ...
            scheduled_exercises = workout_day.get('exercises', [])
            scheduled_exercise_ids = {ex.get('exercise_id') or ex.get('exerciseId') for ex in scheduled_exercises}
            completed_exercise_ids = {ex['exercise_id'] for ex in in_progress_workout.get('exercises', []) if
                                    ex.get('is_completed')}
            ...
            if completed_exercise_ids >= scheduled_exercise_ids:
                ...
                week_number = UserWorkoutService.calculate_week_number(active_plan['start_date'], completed_date)
                ...
                total_duration_seconds = sum(ex.get('duration_seconds', 0) for ex in in_progress_workout['exercises'])
                total_calories_burned = sum(ex.get('calories_burned', 0) for ex in in_progress_workout['exercises'])
                ...
                mongo.db.users.update_one(
                    {"_id": user_id, "workout_history.active_plans.workout_plan_id": workout_plan_id},
