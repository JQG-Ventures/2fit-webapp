...
from app.utils.prompts import assistant_instructions_v2, user_profile_generation, motivational_messages_instructions
...
class ChatService:
    ...
    def handle_message(self, user_message: str = None) -> (str, bool):
        """
        Handles incoming user messages, interacting with the OpenAI API and updating the conversation.

        Args:
            user_message (str): The user's message.

        Returns:
            tuple: The response from the chatbot and a boolean indicating if this is a new conversation.
        """
        ...
        if not conversation:
            ...
            self.update_conversation(
                user_id, conversation_id, assistant_instructions_v2, "system")
        else:
        ...
        response = self.generate_bot_response(conversation_history, self.openai_client, s.OPENAI_MODEL)
    ...
    @staticmethod
    def get_conversation_messages_by_id(conversation_id: str) -> list[dict[str, str]]:
        """
        ...
        """
        result = mongo.db.conversations.find_one(
        ...
        raise KeyError(
            f"Conversation not saved in Mongo for conversation_id {conversation_id}")

    @staticmethod
    def generate_bot_response(conversation_history: list[dict[str, str]], client: OpenAI, model: str) -> str:
        """
    ...
    def generate_motivational_phrases(self):
        ...
        answer = self.generate_bot_response(message_for_bot, self.openai_client, s.INTERNAL_OPENAI_MODEL)
