...
class WorkoutPlan:
    ...
    def delete_workout_plan(plan_id):
        ...
        try:
            result = mongo.db.workout_plans.update_one(
                {"_id": ObjectId(plan_id)}, {"$set": {"is_active": False, "updated_at": datetime.now()}}
            )
            return result.modified_count > 0
        except Exception as e:
            raise Exception(f"Error updating workout plan: {str(e)}")
    ...
    def get_workout_plans_by_muscle_group(muscle_group: str) -> list | None:
        ...
        try:
            plans = list(mongo.db.workout_plans.find({"workout_schedule.exercises.muscle_group": re.compile('^' + re.escape(muscle_group) + '$', re.IGNORECASE)}))
            if not plans:
    ...
    def get_workout_plans_by_difficulty(difficulty: str) -> list | None:
        ...
        try:
            plans = list(mongo.db.workout_plans.find({"level": re.compile('^' + re.escape(difficulty) + '$', re.IGNORECASE)}))
            if not plans:
    ...
    def delete_exercises(plan_id, exercises_to_delete):
        ...
        for day in workout_plan.get("workout_schedule", []):
            ...
            if day_name in exercises_to_delete:
                day["exercises"] = [
                    exercise for exercise in day["exercises"]
        ...
        result = mongo.db.workout_plans.update_one(
            ...
            {"$set": {"workout_schedule": updated_schedule, "updated_at": datetime.now()}}
        )
        return result.modified_count > 0
