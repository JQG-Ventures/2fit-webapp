...
class WorkoutDaySchema(Schema):
    ...
    day_of_week = fields.String(validate=lambda x: x in ['', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'])
...
class WorkoutPlanSchema(Schema):
    ...
    plan_type = fields.String(required=True, validate=lambda x: x in ['library', 'paid', 'personalized', 'challenge'])
    ...
    level = fields.String(default="beginner", validate=lambda x: x in ['beginner', 'intermediate', 'advanced'])
...
#     workout_schedule = fields.List(fields.Nested(WorkoutDaySchema), required=True)  # Daily workout schedule for the challenge
