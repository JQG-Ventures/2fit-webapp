...
class TrainingPreferencesSchema(Schema):
    ...
    available_days = fields.List(fields.String(validate=lambda x: x in ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']), required=True)
    workout_types = fields.List(fields.String(validate=lambda x: x in ["cardio", "strength", "yoga", "dance"]), required=True)
...
class SettingsSchema(Schema):
    ...
    notifications = fields.Nested(NotificationsSchema, missing=lambda: NotificationsSchema().load({}))
...
class ActiveWorkoutPlanSchema(Schema):
    ...
    workout_plan_id = fields.String(required=True)  # Reference to the workout plan
    workout_name = fields.String(required=True)  # Name of the workout plan
    plan_type = fields.String(required=True)  # Plan type: 'library', 'paid', 'personalized', 'challenge'
    start_date = fields.String(required=True)  # When the user started the plan
    end_date = fields.String(allow_none=True)  # Optional end date for challenges or limited-time plans
    is_completed = fields.Boolean(missing=False)  # Track whether the plan has been completed
    progress = fields.Float(missing=0.0)  # Track progress as a percentage (0 to 100)
    last_completed_workout = fields.Nested(ExecutedWorkoutSchema, allow_none=True, missing=None)  # The most recent workout completed
...
class UserSchema(Schema):
    ...
    roles = fields.List(fields.String(), validate=lambda x: x in ["user", "admin"], missing=["user"])
    ...
    auth_provider = fields.String(missing="default", validate=validate.OneOf(["default", "google", "meta", "apple"]))
    fitness_goal = fields.String(required=True, validate=validate.OneOf(["weight", "keep", "strength", "muscle"]))
    fitness_level = fields.String(required=True, validate=validate.OneOf(["beginner", "irregular", "intermediate", "advanced"]))
    ...
    automation_data = fields.Nested(AutomationDataSchema, missing=lambda: AutomationDataSchema().load({}))
    workout_history = fields.Nested(WorkoutHistorySchema, missing=lambda: WorkoutHistorySchema().load({}))
