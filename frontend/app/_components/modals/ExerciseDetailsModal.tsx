...
const ExerciseDetailsModal: React.FC<ExerciseDetailsModalProp> = ({ exercise, onClose, onStartExercise }) => {
					...
					<div className="p-4 flex-1 flex flex-col justify-evenly items-center bg-gradient-to-b from-gray-200 via-gray-500 to-black">
						...
						<div className="w-[95%] flex flex-row justify-around items-center bg-white p-4 rounded-full">
							<div className="flex flex-row items-center">
								...
								<span className="text-lg">{exercise.sets} {capitalizeFirstLetter('sets')}</span>
							</div>
							<div className="flex flex-row items-center">
								<FaFire className="text-black text-xl mr-1" />
								<span className="text-lg">{exercise.reps} {capitalizeFirstLetter('reps')}</span>
							</div>
							<div className="flex flex-row items-center">
								<GiWeightLiftingUp className="text-black text-xl mr-1" />
								<span className="text-lg">{t(`exercise.level.${exercise.difficulty}`) || 'N/A'}</span>
							</div>
						</div>
					</div>
					<button
